 ...
 * @author koo.taejin
 */
public class DefaultApmClientHandler extends SimpleChannelHandler implements ApmClientHandler {
    ...
    private final ApmClientHandlerState state;
    ...
    private final ApmClientFactory clientFactory;
    private SocketAddress connectSocketAddress;
    private volatile ApmClient apmClient;
    ...
    private final ApmClientHandshaker handshaker;
    ...
    public DefaultApmClientHandler(ApmClientFactory clientFactory) {
        this(clientFactory, DEFAULT_PING_DELAY, DEFAULT_ENABLE_WORKER_PACKET_DELAY, DEFAULT_TIMEOUTMILLIS);
    }

    public DefaultApmClientHandler(ApmClientFactory clientFactory, long pingDelay, long handshakeRetryInterval, long timeoutMillis) {
        if (clientFactory == null) {
            throw new NullPointerException("apmClientFactory must not be null");
        }
        
        HashedWheelTimer timer = TimerFactory.createHashedWheelTimer("Apm-ApmClientHandler-Timer", 100, TimeUnit.MILLISECONDS, 512);
        timer.start();
        ...
        this.handshaker = new ApmClientHandshaker(channelTimer, (int) handshakeRetryInterval, maxHandshakeCount);
        
        this.socketId = clientFactory.issueNewSocketId();
        this.pingIdGenerator = new AtomicInteger(0);
        this.state = new ApmClientHandlerState(this, clientFactory.getStateChangeEventListeners());
    ...
    private void prepareChannel(Channel channel) {
        ...
        ApmClientHandlerContext context = new ApmClientHandlerContext(channel, streamChannelManager);
    ...
    public void sendPing() {
        ...
        if (!future.isSuccess()) {
            ...
            throw new ApmSocketException("send ping failed. Error:" + cause.getMessage(), cause);
        }
    ...
    private void await(ChannelFuture channelFuture) {
        ...
        if (channelFuture.isDone()) {
            ...
            if (cancel) {
                ...
                throw new ApmSocketException("io timeout");
            } else {
    ...
    public Future<ResponseMessage> request(byte[] bytes) {
        ...
        if (!isEnable) {
            ...
            closedException.setFailure(new ApmSocketException("invalid state:" + state.getCurrentStateCode() + " channel:" + channel));
    ...
    public ClientStreamChannelContext openStream(byte[] payload, ClientStreamChannelMessageListener messageListener, StreamChannelStateChangeEventHandler<ClientStreamChannel> stateChangeListener) {
        ...
        ApmClientHandlerContext context = getChannelContext(channel);
        ...
        if (message instanceof Packet) {
            ...
            switch (packetType) {
                ...
                case PacketType.APPLICATION_REQUEST:
                    this.messageListener.handleRequest((RequestPacket) message, apmClient);
                    return;
                case PacketType.APPLICATION_SEND:
                    this.messageListener.handleSend((SendPacket) message, apmClient);
                    return;
...
//        }
    ...
    private void ensureOpen() {
        ...
        if (state.isReconnect(currentStateCode)) {
            throw new ApmSocketException("reconnecting...");
        }
        
        throw new ApmSocketException("Invalid socket state:" + currentStateCode);
    }

    // Calling this method on a closed ApmClientHandler has no effect.
    public void close() {
    ...
    public void channelClosed(final ChannelHandlerContext ctx, final ChannelStateEvent e) throws Exception {
        ...
        try {
            ...
            if (needReconnect) {
                clientFactory.reconnect(this.apmClient, this.connectSocketAddress);
            }
        } finally {
            closeResources();
    ...
    private ApmClientHandlerContext getChannelContext(Channel channel) {
        if (channel == null) {
            throw new NullPointerException("channel must not be null");
        }
        return (ApmClientHandlerContext) channel.getAttachment();
    }
    ...
    protected ApmClient getApmClient() {
        return apmClient;
    }
