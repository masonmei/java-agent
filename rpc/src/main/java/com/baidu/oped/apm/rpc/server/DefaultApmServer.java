...
import com.baidu.oped.apm.rpc.common.CyclicStateChecker;
...
import com.baidu.oped.apm.rpc.util.*;
 ...
 * @author Taejin Koo
 */
public class DefaultApmServer implements ApmServer {
    ...
    private final DefaultApmServerState state;
    ...
    public DefaultApmServer(Channel channel, ApmServerConfig serverConfig) {
        this(channel, serverConfig, null);
    }

    public DefaultApmServer(Channel channel, ApmServerConfig serverConfig, ServerStateChangeEventHandler... stateChangeEventListeners) {
        ...
        this.state = new DefaultApmServerState(this, this.stateChangeEventListeners);
    ...
    public void send(byte[] payload) {
        ...
        if (!isEnableDuplexCommunication()) {
            throw new IllegalStateException("Send fail. Error: Illegal State. apmServer:" + toString());
        }
    ...
    public Future<ResponseMessage> request(byte[] payload) {
        AssertUtils.assertNotNull(payload, "payload may not be null.");
        if (!isEnableDuplexCommunication()) {
            throw new IllegalStateException("Request fail. Error: Illegal State. apmServer:" + toString());
        }
    ...
    public void response(int requestId, byte[] payload) {
        ...
        if (!isEnableCommunication()) {
            throw new IllegalStateException("Response fail. Error: Illegal State. apmServer:" + toString());
        }
