...
public abstract class AbstractApmPluginTestSuite extends Suite {
    ...
    private static final String[] REQUIRED_CLASS_PATHS = new String[] {
        ...
        "apm-test", // apm-test-{VERSION}.jar
        "/test/target/classes" // apm-test build output directory
    };
    ...
    public AbstractApmPluginTestSuite(Class<?> testClass) throws InitializationError, ArtifactResolutionException, DependencyResolutionException {
        super(testClass, Collections.<Runner> emptyList());

        ApmAgent agent = testClass.getAnnotation(ApmAgent.class);
        this.agentJar = resolveAgentPath(agent);

        ApmConfig config = testClass.getAnnotation(ApmConfig.class);
    ...
    private String resolveAgentPath(ApmAgent agent) {
        String path = agent == null ? "agent/target/apm-agent-" + Version.VERSION : agent.value();
        String version = agent == null ? Version.VERSION : agent.version();
        String relativePath = path + (!path.endsWith("/") ? "/" : "") + "apm-bootstrap-" + version + ".jar";
    ...
    private String resolveConfigFileLocation(String configFile) {
        ...
        throw new ApmException("Cannot find apm configuration file: " + configFile);
    }
    ...
    protected Statement withBeforeClasses(Statement statement) {
        List<FrameworkMethod> befores= getTestClass().getAnnotatedMethods(BeforeApmPluginTest.class);
        return befores.isEmpty() ? statement : new RunBefores(statement, befores, null);
    }

    @Override
    protected Statement withAfterClasses(Statement statement) {
        List<FrameworkMethod> afters= getTestClass().getAnnotatedMethods(AfterApmPluginTest.class);
    ...
    protected List<Runner> getChildren() {
        ...
        try {
            for (int ver : jvmVersions) {
                ...
                // TODO for now, java 8 is not mandatory to build apm.
                ...
                ApmPluginTestContext context = new ApmPluginTestContext(agentJar, configFile, requiredLibraries, getTestClass().getJavaClass(), testClassLocation, jvmArguments, debug, ver, javaExe); 
                
                List<ApmPluginTestInstance> cases = createTestCases(context);
                
                for (ApmPluginTestInstance c : cases) {
                    runners.add(new ApmPluginTestRunner(context, c));
                }
            }
        } catch (Exception e) {
    ...
    protected abstract List<ApmPluginTestInstance> createTestCases(ApmPluginTestContext context) throws Exception;
}
