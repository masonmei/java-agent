 ...
 *
 */
public class ApmPluginTestStatement extends Statement implements ApmPluginTestConstants {
    private static final String JUNIT_OUTPUT_DELIMITER_REGEXP = Pattern.quote(JUNIT_OUTPUT_DELIMITER);

    private final ApmPluginTestRunner runner;
    private final RunNotifier notifier;
    private final ApmPluginTestInstance testCase;
    private final ApmPluginTestContext context;
    private final Result result = new Result();
    
    public ApmPluginTestStatement(ApmPluginTestRunner runner, RunNotifier notifier, ApmPluginTestContext context, ApmPluginTestInstance testCase) {
        this.runner = runner;
    ...
    private String[] buildCommand() {
        ...
        list.add("-Dapm.agentId=build.test.0");
        list.add("-Dapm.applicationName=test");
        list.add("-D" + ApmPluginTestConstants.APM_TEST_ID + "=" + testCase.getTestId());
        ...
        if (context.getConfigFile() != null) {
            list.add("-Dapm.config=" + context.getConfigFile());
        }
    ...
    private String getAgent() {
        return "-javaagent:" + context.getAgentJar() + "=bootClass=com.baidu.oped.apm.test.PluginTestAgent";
    }
    ...
    private ApmPluginTestException toException(String message, String exceptionClass, List<String> traceInText) {
        ...
        for (int i = 0; i < traceInText.size(); i++) {
            ...
            if (trace.equals("$CAUSE$")) {
                ApmPluginTestException cause = toException(traceInText.get(i + 2), traceInText.get(i + 1), traceInText.subList(i + 3, traceInText.size()));
                return new ApmPluginTestException(exceptionClass + ": " + message, Arrays.copyOf(stackTrace, i), cause);
            }
        ...
        return new ApmPluginTestException(exceptionClass + ": " + message, stackTrace);
    }
}
